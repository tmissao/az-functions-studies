...
# resource "azurerm_role_assignment" "roleassignment_functionapp_hoststorage" {
#   for_each = toset(local.host_storage_required_permissions)
...
resource "azurerm_role_assignment" "current_user_hoststorage" {
  for_each = toset(local.host_storage_required_permissions)
  scope                = azurerm_storage_account.this.id
  role_definition_name = each.key
  principal_id         = data.azurerm_client_config.current.object_id
